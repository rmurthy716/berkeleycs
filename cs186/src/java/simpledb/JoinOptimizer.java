...
public class JoinOptimizer {
    ...
    HashMap<Integer, Set<Set<LogicalJoinNode> > > subsets;
    ...
    public JoinOptimizer(LogicalPlan p, Vector<LogicalJoinNode> joins) {
        this.p = p;
        this.joins = joins;
	subsets = new HashMap<Integer, Set<Set<LogicalJoinNode> > >();
    }
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
	    ...
	    double ret = cost1 + (double)((cost2 + card2) * card1);
            return ret;
        }
    }
    ...
    /* Simple wrapper for memoization */
    private Set<Set<LogicalJoinNode> > getSub(int i) {
	if (!subsets.containsKey(i)) {
	    subsets.put(i, enumerateSubsets(joins, i));
	    return subsets.get(i);
	}
	else return subsets.get(i);
    }
	...
	for (int i = 1; i <= joins.size(); i++) {
	    for (Set<LogicalJoinNode> s : getSub(i)) {
		CostCard bestPlan = new CostCard();
		bestPlan.cost = Double.MAX_VALUE;
		bestPlan.card = Integer.MAX_VALUE;
		bestPlan.plan = new Vector<LogicalJoinNode>();
		for (LogicalJoinNode rem : s) {
		    HashSet<LogicalJoinNode> t
			= new HashSet<LogicalJoinNode>(s);
		    t.remove(rem);
		    CostCard result 
			= computeCostAndCardOfSubplan(stats,
						      filterSelectivities,
						      rem,
						      t,
						      bestPlan.cost,
						      optjoin);
		    if (result != null &&
			result.cost < bestPlan.cost) bestPlan = result;
		}
		optjoin.addPlan(s,
				bestPlan.cost,
				bestPlan.card,
