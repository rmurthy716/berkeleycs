...
public class Aggregate extends Operator {
	...
	this.gfield = gfield;
	this.aop = aop;
	TupleDesc mytd = child.getTupleDesc();
	Type gbfieldtype = null;
	if (gfield != Aggregator.NO_GROUPING)
	    gbfieldtype = mytd.getFieldType(gfield);
	Aggregator myagg = (mytd.getFieldType(afield) == Type.INT_TYPE) 
	    ? new IntegerAggregator(gfield, gbfieldtype, afield, aop)
	    : new StringAggregator(gfield, gbfieldtype, afield, aop);
	try {
	    this.child.open();
	    while (this.child.hasNext()) 
		myagg.mergeTupleIntoGroup(this.child.next());
	    this.output = myagg.iterator();
	    this.output.open();
	}
	catch (Exception e) {}
    }
